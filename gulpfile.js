...
async function prepareFontsFolder() {
  await fs.promises.mkdir(path.build.fonts, { recursive: true });
}
...
async function webp() {
    ...
    return gulp
        ...
        .pipe(gulpIf(isImageFile, webp({ quality: 100 }))) 
...
async function clean() {
  await del([path.build.root + '*'], { force: true });
}
...
const fonts = gulp.series(prepareFontsFolder, ttfToWoff, ttfToWoff2, fontFace);
const images = gulp.series(img, webp);
const mainTasks = gulp.series(clean, gulp.parallel(html, libs, style, js, images, svg), fonts);
const mainTasksBuild = gulp.series(clean, gulp.parallel(htmlBuild, fonts, libs, style, js, images, svg));
const dev = gulp.series(mainTasks, gulp.parallel(watchFiles, server));
const build = gulp.series(clean, mainTasksBuild, minifyCSS, minifyJs, gulp.parallel(watchFiles, server));
